function multilayer_fuzzy_network_preassigned_time_sync()
% 基于扩充邻接矩阵的三层T-S模糊多层网络抗欺骗攻击指定时间同步仿真
...
fprintf('=== 基于扩充邻接矩阵的三层T-S模糊网络指定时间同步仿真 ===\n');
fprintf('间歇控制周期数: %d\n', N_periods);
fprintf('时间步长: %.4f\n', h);
%% ==================== 网络参数 ====================
M = 3; % 层数
...
% 计算 m2 参数
fprintf('\n=== 计算理论参数用于指定时间同步 ===\n');
beta_min = min(beta_theta);
fprintf('β_min = %.4f\n', beta_min);
T_max_theory = 2.6780; % 设置一个默认值
%% ==================== 指定时间参数设置 ====================
T_p = 1.2; % 指定收敛时间（小于理论固定时间）
time_ratio = T_max_theory / T_p; % 指定时间同步系数
fprintf('\n=== 指定时间同步参数 ===\n');
fprintf('固定时间理论收敛时间 T_max = %.4f 秒\n', T_max_theory);
fprintf('指定收敛时间 T_p = %.4f 秒\n', T_p);
fprintf('时间比例系数 T_max/T_p = %.4f\n', time_ratio);
%% ==================== 初始条件设置 ====================
% 同步态初始条件
...
%% ==================== 主仿真循环 ====================
fprintf('开始基于扩充邻接矩阵的抗攻击指定时间同步仿真循环...\n');
for k = 1:N_periods
%% ===== 控制激活阶段 =====
...
% 指定时间控制器：在beta和gamma项前加上时间比例系数
phi1_val1 = -alpha_theta(1)*q_val - time_ratio*beta_theta(1)*sign(q_val) - time_ratio*gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
phi1_val2 = -alpha_theta(2)*q_val - time_ratio*beta_theta(2)*sign(q_val) - time_ratio*gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
...
phi2_val1 = -alpha_theta(1)*q_val - time_ratio*beta_theta(1)*sign(q_val) - time_ratio*gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
phi2_val2 = -alpha_theta(2)*q_val - time_ratio*beta_theta(2)*sign(q_val) - time_ratio*gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
...
phi3_val1 = -alpha_theta(1)*q_val - time_ratio*beta_theta(1)*sign(q_val) - time_ratio*gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
phi3_val2 = -alpha_theta(2)*q_val - time_ratio*beta_theta(2)*sign(q_val) - time_ratio*gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
...
T_p_rounded = round(T_p, 4); % 指定时间的四舍五入值
...
stairs(t_attack, attack_events, 'Color', [0, 0, 0.8], 'LineWidth', 0.01);
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$\phi^{(1)}_{i1}(t)$', '$\phi^{(1)}_{i2}(t)$', '$\phi^{(1)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$U^{(1)}_{i1}(t)$', '$U^{(1)}_{i2}(t)$', '$U^{(1)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$\phi^{(2)}_{i1}(t)$', '$\phi^{(2)}_{i2}(t)$', '$\phi^{(2)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$U^{(2)}_{i1}(t)$', '$U^{(2)}_{i2}(t)$', '$U^{(2)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$\phi^{(3)}_{i1}(t)$', '$\phi^{(3)}_{i2}(t)$', '$\phi^{(3)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$U^{(3)}_{i1}(t)$', '$U^{(3)}_{i2}(t)$', '$U^{(3)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
xlabel('$t$', 'Interpreter', 'latex');xlim([0, 2]);box on;
ylabel('$U^{(1)}_i(t),i=1,2,...,6$', 'Interpreter', 'latex');
grid on;
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$q(e^{(1)}_{i1}(t))$', '$q(e^{(1)}_{i2}(t))$', '$q(e^{(1)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$q(e^{(2)}_{i1}(t))$', '$q(e^{(2)}_{i2}(t))$', '$q(e^{(2)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% Add T_p marker for preassigned-time
p = plot(T_p, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$q(e^{(3)}_{i1}(t))$', '$q(e^{(3)}_{i2}(t))$', '$q(e^{(3)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}^1_p=%.1f$', T_p_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
...
% 创建保存目录
save_dir = 'simulation_results_pdf';
