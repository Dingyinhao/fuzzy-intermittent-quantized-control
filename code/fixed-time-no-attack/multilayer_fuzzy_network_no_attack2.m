function multilayer_fuzzy_network_no_attack2()
% 基于扩充邻接矩阵的三层T-S模糊多层网络无攻击固定时间同步仿真
% 第一层：Liu系统，第二层：Lorenz系统，第三层：Rossler系统
% 使用扩充邻接矩阵 \breve{\mathcal{A}}^{(k)} 和新的状态迭代公式
close all; clear; clc;
% === 确保完全可重现性 ===
rng(42, 'twister'); % 固定所有随机数生成器
rand('state', 42);
randn('state', 42);
%% ==================== 间歇控制时间设置 ====================
n = 480; % 时间细分参数
...
fprintf('=== 基于扩充邻接矩阵的三层T-S模糊网络固定时间同步仿真（无攻击） ===\n');
fprintf('间歇控制周期数: %d\n', N_periods);
fprintf('时间步长: %.4f\n', h);
%% ==================== 网络参数 ====================
M = 3; % 层数
...
alpha_theta = [6.5, 6.6]; % 基本控制 α_θ
beta_theta = [14, 14.1]; % 符号函数 β_θ
gamma_theta = [5.7, 5.8]; % 幂次项 γ_θ
delta = 1.2; % 幂次指数 δ > 1 for fixed-time
...
Lambda = 0.8; % 间歇控制参数
fprintf('=== 控制参数验证（无攻击） ===\n');
fprintf('控制增益: α_min=%.2f, β_min=%.2f, γ_min=%.2f\n', alpha, beta, gamma);
fprintf('量化参数: ρ=%.2f, χ=%.4f\n', rho, chi);
%% ==================== 构造扩充邻接矩阵 ====================
fprintf('\n=== 构造扩充邻接矩阵 ===\n');
% 调整后的Laplace矩阵L_A1
...
% 控制输入相关变量（无攻击）
u1 = zeros(3*N_original, total_time_steps+1); % 第一层控制输入
u2 = zeros(3*N_original, total_time_steps+1); % 第二层控制输入
u3 = zeros(3*N_original, total_time_steps+1); % 第三层控制输入
% 同步态
...
% 按照推论1的控制器公式（无攻击）
u1_val1 = -alpha_theta(1)*q_val - beta_theta(1)*sign(q_val) - gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
u1_val2 = -alpha_theta(2)*q_val - beta_theta(2)*sign(q_val) - gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
% 应用模糊权重
...
u2_val1 = -alpha_theta(1)*q_val - beta_theta(1)*sign(q_val) - gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
u2_val2 = -alpha_theta(2)*q_val - beta_theta(2)*sign(q_val) - gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
...
u3_val1 = -alpha_theta(1)*q_val - beta_theta(1)*sign(q_val) - gamma_theta(1)*sign(q_val)*abs(q_val)^delta;
u3_val2 = -alpha_theta(2)*q_val - beta_theta(2)*sign(q_val) - gamma_theta(2)*sign(q_val)*abs(q_val)^delta;
...
rho = 0.49;  % 噪声强度系数（单独使用）
fprintf('Lipschitz常数 L = %.4f\n', L);
fprintf('噪声强度系数 ρ = %.4f\n', rho);
...
%% ==================== 计算论文中的参数（无攻击情况，根据Corollary 1） ====================
% m1' = α(1-χ) - λ_max(Ξ) - ρ/2
m1_prime = alpha * (1 - chi) - lambda_max_Xi - rho/2;

% m2' = β（这是系数，实际使用时会乘以误差的L1范数）
m2_prime = beta;

% m3' = γ(nMN)^((1-δ)/2)(1-χ)^δ  
m3_prime = gamma * (n_dim*M*N_original)^((1-delta)/2) * (1-chi)^delta;

% m4' = α(1-χ)
m4_prime = alpha * (1 - chi);
...
%% ==================== 理论条件验证（根据Corollary 1） ====================
% 条件: α(1-χ) > λ_max(Ξ) + ρ/2
condition_lhs = alpha * (1 - chi);
condition_rhs = lambda_max_Xi + rho/2;
condition_satisfied = condition_lhs > condition_rhs;

fprintf('\n=== 条件验证（无攻击） ===\n');
fprintf('条件: α(1-χ) = %.4f > λ_max(Ξ) + ρ/2 = %.4f: %s\n', ...
    condition_lhs, condition_rhs, string(condition_satisfied));

% 收敛条件: m1' - (m1'+m4')(1-Λ) > 0
convergence_condition = m1_prime - (m1_prime + m4_prime)*(1-Lambda);
fprintf('收敛条件 m1'' - (m1''+m4'')(1-Λ) = %.4f > 0: %s\n', ...
    convergence_condition, string(convergence_condition > 0));

%% ==================== 理论收敛时间估计（无攻击） ====================
if convergence_condition > 0 && condition_satisfied
    % 根据Corollary 1的公式
    % T_max^2 = δ/[(m1'-(m1'+m4')(1-Λ))(δ-1)] * ln(1 + m1'/m2' * (m2'/m3')^(1/δ))
    
    % 估计m2'的实际值（需要考虑同步误差的L1范数）
    % 这里使用一个合理的估计值
    m2_estimate = m2_prime * 1.0;  % 简化估计
    
    if m2_estimate > 0 && m3_prime > 0
        numerator = delta;
        denominator = convergence_condition * (delta - 1);
        log_term = log(1 + (m1_prime/m2_estimate) * (m2_estimate/m3_prime)^(1/delta));
        T_max_theory = numerator / denominator * log_term;
        
        fprintf('\n理论固定时间上界（无攻击）: T_max^2 = %.4f 秒\n', T_max_theory);
    else
...
%% ==================== 修正的calculate_Xi函数（嵌入版本） ====================
function Xi = calculate_Xi(L, L_B, F, L_A, C, E, d, c, n, M, N)
    % 严格按照论文公式计算 Xi（不包含rho）
    % Xi = [(L ⊗ I_{nMN}) - d(I_N ⊗ (L_B ⊗ F)) - c((L_A + C) ⊗ E)]^{sym}
    
    % 第一项: L ⊗ I_{nMN}（L是标量Lipschitz常数）
    I_nMN = eye(n * M * N);
    term1 = L * I_nMN;
    ...
    if size(L_A, 1) == N * M && size(L_A, 2) == N * M
        ...
        % 如果L_A是单层的，需要扩展
        error('L_A维度不正确，应该是 %d×%d 的块对角矩阵', N*M, N*M);
    end
    
    % 确保维度匹配
    if size(term1,1) ~= n*M*N || size(term2,1) ~= n*M*N || size(term3,1) ~= n*M*N
        error('矩阵维度不匹配！');
    end
    ...
    % 验证对称性
    if norm(Xi - Xi', 'fro') > 1e-10
        warning('Xi矩阵对称性误差较大: %.2e', norm(Xi - Xi', 'fro'));
    end
end
%% ==================== 结果分析与可视化 ====================
T_2 = T_max_theory;
...
fprintf('最终同步误差范数:\n');
fprintf('第一层 (Liu): %.6f\n', final_e1_norm);
fprintf('第二层 (Lorenz): %.6f\n', final_e2_norm);
fprintf('第三层 (Rossler): %.6f\n', final_e3_norm);
%% ==================== 图形绘制 ====================
%% 第一层控制输入信号
figure(1);
hold on;
...
legend_entries = {'$U^{(1)}_{i1}(t)$', '$U^{(1)}_{i2}(t)$', '$U^{(1)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)};
...
T_2_rounded = round(T_max_theory, 4);
p = plot(T_max_theory, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$U^{(2)}_{i1}(t)$', '$U^{(2)}_{i2}(t)$', '$U^{(2)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
else
...
T_2_rounded = round(T_max_theory, 4);
p = plot(T_max_theory, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 8);
legend_entries = {'$U^{(3)}_{i1}(t)$', '$U^{(3)}_{i2}(t)$', '$U^{(3)}_{i3}(t)$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)};
legend([h1, h2, h3, p], legend_entries, 'Location', 'northeast', 'Interpreter', 'latex');
else
...
legend([h1, h2, h3, p], {'$q(e^{(1)}_{i1}(t))$', '$q(e^{(1)}_{i2}(t))$', '$q(e^{(1)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)}, 'Location', 'northeast', 'Interpreter', 'latex');
else
...
T_2_rounded = round(T_max_theory, 4);
p = plot(T_max_theory, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 6);
legend([h1, h2, h3, p], {'$q(e^{(2)}_{i1}(t))$', '$q(e^{(2)}_{i2}(t))$', '$q(e^{(2)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)}, 'Location', 'northeast', 'Interpreter', 'latex');
else
...
T_2_rounded = round(T_max_theory, 4);
p = plot(T_max_theory, 0, 'ro', 'MarkerFaceColor', 'red', 'MarkerSize', 6);
legend([h1, h2, h3, p], {'$q(e^{(3)}_{i1}(t))$', '$q(e^{(3)}_{i2}(t))$', '$q(e^{(3)}_{i3}(t))$', sprintf('$\\hat{\\mathcal{T}}_{\\max}^2=%.4f$', T_2_rounded)}, 'Location', 'northeast', 'Interpreter', 'latex');
else
...
%% ==================== 自动保存PDF功能 (固定时间同步版本) ====================
fprintf('\n=== 开始保存图形为PDF文件 (固定时间同步版本) ===\n');

% 创建保存目录
save_dir = 'simulation_results_fixed_time_pdf';
